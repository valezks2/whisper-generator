<script lang="ts">
	...
	function drawImageAndText() {
		...
		if (lines[0]) {
			ctx.font = `90px Upright, sans-serif`;
			ctx.fillStyle = 'white';
...
<div class="flex flex-col min-h-screen font-upright bg-white">
	...
	<main class="flex-1 flex flex-col items-center justify-center px-4">
		...
		>
			...
			<div
				class="mt-3 mb-1 flex gap-8 justify-center"
				style="display:{showButtons ? 'flex' : 'none'}"
			>
				<button
					class="bg-white border-2 border-black font-upright text-[1.5rem] text-[#202020] px-5 py-2 transition-colors duration-300 hover:bg-[#f5f5f5] hover:border-[#505050] hover:text-[#505050] cursor-pointer"
			...
			<div class="text-center mt-4 image-container" style="display:{img?.src ? 'block' : 'none'}">
				<canvas id="canvas" bind:this={canvas} class="max-w-full border-2 border-black"></canvas>
			</div>
		</div>
	</main>
